...
app.post("/refresh" ,async(req, res) => {
  ...
  jwt.verify(refreshToken , RREFRESH_KEY ,  async (err, user) => {
    ...
    try{
      const newAccessToken = jwt.sign({ id: user.id }, SECRET_KEY ,{ expiresIn: '3s'})
      const newRefreshToken = jwt.sign({ id: user.id }, RREFRESH_KEY , { expiresIn: '1800s'})

    if(!(newAccessToken && newRefreshToken)) return res.status(401).json("Сheck your login details")
     
    const UserTokenFind = db.userToken.findOneAndUpdate({userId:user.id} , {
      userId:user.id,
    ...
    if(!UserTokenFind)  return res.status(403).json("Refresh token is not valid!");
     ...
     await db.userToken.findOneAndUpdate({userId:user.id} , {
      userId:user.id,
      token: newRefreshToken
    })
  ...
  .then(async user => {    
      if (!user) {
          ...
          if (!Bcrypt.compareSync(password, user.password)) {
              ...
              const token = jwt.sign({ id: user._id}, SECRET_KEY ,{ expiresIn: '18s'})
              const refreshToken = jwt.sign({ id: user._id}, RREFRESH_KEY , { expiresIn: '18000s'})
